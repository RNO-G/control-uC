 ...
 * Modified by Cosmin Deaconu <cozzyd@kicp.uchicago.edu> to use hal_timer (since RTC is being used for other tasks)
 *
...
static void timer_callback(const struct timer_task * const task) 
{
  RtcAlarmIrq(); 
}

static struct timer_task task = {.cb = timer_callback, .mode = TIMER_TASK_ONE_SHOT }; 

void RtcInit( void )
{
    if( RtcInitialized == false )
    {
        //this uses 32.768 clock, so this results in 1 ms
...
{
    RtcTimerContext.Time = LORA_TIMER.time; 
...
{
    return LORA_TIMER.time; 
}

uint32_t RtcGetTimerElapsedTime( void )
{
    return ( uint32_t)( LORA_TIMER.time - RtcTimerContext.Time );
}
